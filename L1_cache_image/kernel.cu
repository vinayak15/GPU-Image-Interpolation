//Interpolation using CUDA and OpenCV and Optimization using L1 cache
//Here there is not much code difference as compared to register memory Optimization 
//Only Differnece is to set preffered memory in L1 cache 
...
#include<vector>
void psnr(Mat, Mat);
__global__ void lneigh(unsigned char *, const unsigned char *, int, int);
...
int main()
{
	...
	for (int i = 0, a = 0; i < img1.rows && a < img1.rows / 2; i += 2, a++)			//Shrinking of image to half of its size
	{
	...
	cudaFuncSetCacheConfig(bilinear1, cudaFuncCachePreferL1);				//Cache optimazation

//	lneigh << < gridDim,blockDim>> >( cuda_new_img, cuda_img , img1.rows , img1.cols);
//Zooming image usingBi Linear interpolation technique 


	bilinear1 << <gridDim, blockDim >> >(cuda_new_img, cuda_img, img1.rows, img1.cols);
	cudaFuncSetCacheConfig(bilinear2, cudaFuncCachePreferL1);			//Cache optimazation

	bilinear2 << <gridDim, blockDim >> >(cuda_new_img, cuda_img, img1.rows, img1.cols);
	cudaFuncSetCacheConfig(bilinear3, cudaFuncCachePreferL1);			//Cache optimazation

//Bilinear interpolation and Optimization wth register memory
//Bi linear intepolation is executed in three phase and register memory optimization occurs in third phase only 
...
__global__ void bilinear3(unsigned char *new_image, const unsigned char *image, int rows, int cols)
{
	...
	col *= 2;
	unsigned char a, b;
	a = new_image[row*cols + col];							//optimization through shared memery
